...
// Mexico City: ONE bundle from SEMOVI with every operator in it, told apart by
// agency_id — the concessioned corridors (CC) and RTP buses navy, the twelve
// STE TROLLEYBUS lines green, the seven Metrobús BRT corridors amber (the
// treatment Rio's BRT and Cairo's paratransit share), and on the fixed-track
// side the Metro, the Tren Ligero, the Ferrocarril Suburbano, the Tren El
// Insurgente and the three Cablebús gondolas, each in its official colour.
// Every operator numbers its lines from 1, so line keys carry the operator's
// initial (see lineKey) — the rule Sofia's ТБ/ТМ prefixes set.
...
// ONE national-format feed + ONE synthesized one, TWO cfgs.
//
// BODS (DfT Bus Open Data) publishes a regional GTFS for "london": every
// TfL-contracted bus with the number the street signs, and — a gift no other
// city here got — the Underground as route_type 1, the DLR as 2, the Tramlink
// as 0 and the IFS Cloud Cable Car as 6, all in the same bundle. Excluded:
// long-distance coaches (route_type 200 — National Express, FlixBus and
// Berry's would draw motorways to Manchester) and the 73 "Replacement
// Service" routes (rail-replacement buses; the family's Budapest pótló rule).
//
// The Overground's six named lines and the Elizabeth line are National Rail
// and absent from BODS — pipeline/rail-feed.mjs synthesizes data/gtfs-rail
// for them from the TfL Unified API (ordered station sequences). Both rail
// feeds ship geometry too coarse to draw (BODS shapes are simplified,
// the TfL API draws station-to-station chords), so the HMM matches
// everything onto the OSM track graph and the printed geometry is OSM's.
//
// LINE KEYS: nothing to invent. Buses print their street number (1, N29,
// SL8, X140), the railways print their names (Bakerloo, DLR, Lioness,
// Elizabeth line) — the keys ARE the street's own words, so the bare-numbers
// table stays almost empty (one cosmetic shortening, below).
const LBL = new Map();
...
const isCoach = (r) => (r.route_type || '').trim() === '200';
const isReplacement = (r) => /replacement/i.test(r.agency_id ? (AGENCY_NAME.get(r.agency_id) || '') : '');
// agency.txt is read once up front: BODS identifies the rail-replacement
// pseudo-operator only by its agency NAME
const AGENCY_NAME = new Map();
for (const a of await readCsv(join(ROOT, 'data/gtfs/agency.txt'))) {
  AGENCY_NAME.set(a.agency_id, (a.agency_name || '').trim());
}

const MODES = [{
  mode: 'bus', label: 'buses', graphMode: 'road',
  // Greater London and its fringe towns — 5 × 5 Overpass tiles, the São Paulo
  // treatment (one box always times out)
  osmFiles: Array.from({ length: 25 }, (_, i) => `data/osm/tiles/t${i + 1}.json`),
  color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['88']),
  feeds: [
    { tag: 'bods', dir: 'data/gtfs', routeTypes: ['3'],
      skipRoute: (r) => isCoach(r) || isReplacement(r),
      mapKey: (sn) => sn || null },
  ],
}];
const tramAll = tramLines.length === 1 && tramLines[0] === 'all';
// Everything on a fixed guideway gets the trunk treatment (wide ribbon,
// station discs, always-on names): the Underground, the DLR, the Overground,
// the Elizabeth line, the Tramlink and the cable car.
const isRailTrunk = (l) => l in LON_COLORS;
if (tramAll || tramLines.length) MODES.push({
  mode: 'tram', label: 'Underground, DLR, Overground, Elizabeth line & Tram',
  osmFiles: Array.from({ length: 6 }, (_, i) => `data/osm/rail-tiles/r${i + 1}.json`),
  graphMode: 'tram', railKeep: new Set(['subway', 'light_rail', 'rail', 'tram']),
  // the cable car is aerialway=cable_car in OSM, not railway at all
...
async function processMode(cfg) {
  ...
  for (const feed of cfg.feeds) {
    ...
    for await (const t of iterCsv(join(fdir, 'trips.txt'))) {
      ...
      if (e.trips.length < (feed.allVariants ? 1e9 : tripCap)) e.trips.push({ trip_id: t.trip_id, headsign: t.trip_headsign });
    }
